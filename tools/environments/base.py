        ...
        try:
            ...
            while proc.poll() is None:
                ...
                if (
                    ...
                    drain_thread.join(timeout=2)
                    prompt_msg = (
                        "\n[Command terminated: Process appeared to hang waiting for user input / interactive prompt "
                        f"('{_prompt_match}'). "
                        "Terminal tool executions are non-interactive. Use non-interactive flags (e.g. -y, --yes) or pipe input via stdin.]"
                    )
                    return self._finalize_wait_result(
                        output,
                        output.render(suffix=prompt_msg),
                        124,
                    )
                ...
                if effective_inactivity is not None and _idle_time >= effective_inactivity:
                    ...
                    return self._finalize_wait_result(
                        output,
                        output.render(suffix=inactivity_msg),
                        124,
                    )
