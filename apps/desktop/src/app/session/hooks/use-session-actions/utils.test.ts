...
describe('dedupeInflightUserAgainstTranscript', () => {
  it('suppresses an in-flight user already present as the runtime and persisted tail', () => {
    const runtime = [
      msg('runtime-user', 'user', 'earlier prompt', { timestamp: 1 }),
      msg('runtime-assistant', 'assistant', 'earlier answer', { timestamp: 2 }),
      msg('runtime-current', 'user', 'current prompt', { timestamp: 3 })
    ]

    const deduped = dedupeInflightUserAgainstTranscript(runtime, runtime, runningProjection('current prompt'))
    const restored = appendLiveSessionProjection(runtime, deduped)
