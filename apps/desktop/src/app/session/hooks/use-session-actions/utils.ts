...
export function appendLiveSessionProjection(messages: ChatMessage[], projection: LiveSessionProjection): ChatMessage[] {
  ...
  const persistedInLatestRun = (text: string): boolean =>
    latestUserRun.some(
      message => comparableUserMessageText(chatMessageText(message)) === comparableUserMessageText(text)
    )
...
export function dedupeInflightUserAgainstTranscript(
  ...
  if (runtimeMessages.length) {
    ...
    // Some gateway resume payloads already include the current accepted user
    // row in `messages` while also exposing that same row through `inflight`.
    // When the shared tail anchor is that final user row, it is stronger
    // evidence than the persisted-suffix check below: both authorities point
    // at the same still-open turn, so rendering the projection would duplicate
    // it (not preserve an intentionally repeated historical prompt).
    if (
      persistedAnchorIndex === persistedMessages.length - 1 &&
      runtimeAnchor.role === 'user' &&
      normalizedMessageText(runtimeAnchor) === inflightUser
    ) {
