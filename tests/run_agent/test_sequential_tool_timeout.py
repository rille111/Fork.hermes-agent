...
def test_timed_out_mutation_fences_later_mutations_until_worker_finishes(
    ...
    agent = _make_agent(tmp_path)
    agent.session_id = "shared-mutation-session"
    first_started = threading.Event()
    release_first = threading.Event()
    dispatched: list[str] = []
    terminal_events: list[dict] = []
    ...
    def _dispatch(_name, _args, _task_id, *, tool_call_id, **_kwargs):
        dispatched.append(tool_call_id)
        if tool_call_id == "hung-write":
            first_started.set()
            release_first.wait()
            return "late mutation result"
        return "later mutation result"

    calls = [
        _tool_call(
            "hung-write",
            "write_file",
            json.dumps({"path": str(tmp_path / "first.txt"), "content": "first"}),
        ),
        _tool_call(
            "blocked-patch",
            "patch",
    ...
    def _flush_and_rotate_session(flushed_messages):
        if (
            flushed_messages
            and flushed_messages[-1].get("tool_call_id") == "hung-write"
        ):
            agent.session_id = "compressed-child-session"
        return True

    agent._flush_messages_to_session_db.side_effect = _flush_and_rotate_session
    ...
    try:
        with (
            ...
            replacement_agent = _make_agent(tmp_path)
            replacement_agent.session_id = agent.session_id
                ...
                SimpleNamespace(
                    tool_calls=[
                        _tool_call(
                            ...
                            json.dumps(
                                {
                                    "path": str(tmp_path / "replacement.txt"),
                                    "content": "replacement",
                                }
                            ),
                        )
            ...
            assert dispatched == ["hung-write"]
            assert agent.session_id == "compressed-child-session"
            assert replacement_messages[0]["effect_disposition"] == "none"
            assert "was not started" in replacement_messages[0]["content"]

            release_first.set()
            import agent.tool_executor as tool_executor

            with tool_executor._PENDING_FILE_MUTATION_WORKERS_LOCK:
                pending = next(iter(tool_executor._PENDING_FILE_MUTATION_WORKERS))
