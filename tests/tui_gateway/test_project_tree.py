...
def test_persisted_git_evidence_promotes_and_collapses_the_path_fallback_lane():
    # Historical rows may disagree: one has only a cwd (path fallback), while a
    # newer row persisted the common git root. Both now use the canonical main
    # branch lane id, so the rows collapse without losing the stronger Git
    # capability. The fallback's basename label remains intact, which prevents
    # the renderer from mistaking it for a branch target.
    sessions = [
        _session("/repo"),
    ...
    assert repo["groups"][0]["id"] == "/repo::branch::main"
    assert repo["groups"][0]["label"] == "repo"
