...
_FOOTER_INJECTION_RE = re.compile(r"(?i)(?:MEDIA:|FILE:|ATTACHMENT:|!\[)")
...
class MutationIdentity:
    ...
    def display_path(self) -> str:
        return self.display if self.display is not None else self.path
...
class TurnFileMutationVerifier:
        ...
        *,
        ...
        if raw_landed:
            ...
            for raw_path in landed_paths:
                self._clear_landed_path_aliases(raw_path, effective_task_id)
            return
    ...
    def _clear_landed_path_aliases(self, raw_path: str, task_id: str) -> None:
        """Clear unresolved aliases for a path the registry reports as landed."""
        ident = self._identity_for_path(raw_path, task_id)
        if ident is None:
            return

        keys_to_remove = {ident.key()}
        if ident.backend_kind == "local":
            canonical = _canonical_observation_key(raw_path, ident.task_scope)
            if canonical is not None:
                for key, entry in list(self._ledger.items()):
                    candidate = entry.identity
                    if (
                        candidate.backend_kind != "local"
                        or candidate.authority != ident.authority
                        or candidate.task_scope != ident.task_scope
                    ):
                        continue
                    if (
                        _canonical_observation_key(
                            candidate.path, candidate.task_scope
                        )
                        == canonical
                    ):
    ...
    def _identity_for_path(
        ...
        if display != _UNKNOWN_MUTATION_TARGET and dialect == _path_dialect():
    ...
    def _capture_baseline(
        ...
        *,
        ...
        if ident is None or ident.backend_kind != "local":
            return None
        resolved = _resolve_local_path(raw_path, task_id)
        if resolved is None or not _path_allowed_for_observation(str(resolved)):
            return None
        with self._lock:
            generation = self._budget.generation
            if turn_generation is not None and turn_generation != generation:
                return None
        ...
        with self._lock:
            if self._budget.generation != generation:
                return None
...
def _default_resolve_backend(task_id: str) -> Tuple[str, str, str]:
    ...
    if env is None:
        ...
        try:
            ...
            env_type = _terminal_env_type_for_task(task_id or "default")
        except Exception:
        ...
        try:
            ...
            workspace = (
                overrides.get("cwd")
                or get_session_cwd(task_id or "default")
                or config.get("cwd")
                or ""
            )
        except Exception:
        ...
        if env_type == "ssh":
            endpoint = ":".join(
                str(config.get(key) or os.getenv(env_key, ""))
                for key, env_key in (
            ...
            return "ssh", f"configured:ssh:{endpoint}:{workspace}", "posix"
        if env_type in {"docker", "singularity", "modal", "daytona"}:
            image = config.get(f"{env_type}_image", "")
            return "container", f"configured:{env_type}:{image}:{workspace}", "posix"
        return "unknown", f"configured:{env_type}:{workspace}", "unknown"
    cls = type(env).__name__
    if cls == "LocalEnvironment":
        return "local", "host", _path_dialect()
    authority = f"{cls}:{id(env):x}:{getattr(env, 'cwd', '') or ''}"
    if "SSH" in cls:
        return "ssh", authority, "posix"
    if "Docker" in cls or "Singularity" in cls or "Modal" in cls or "Daytona" in cls:
        return "container", authority, "posix"
    return "unknown", authority, "unknown"
...
def _sanitize_footer_path(path: str) -> str:
    cleaned = _sanitize_footer_text(str(path)).replace("`", "'")
    ...
    *,
    ...
    try:
        ...
        for path, info in failed.items():
            if shown >= 10:
                break
            path = _sanitize_footer_path(path)
