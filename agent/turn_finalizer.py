    ...
    *,
    ...
    try:
        ...
        if final_response and not interrupted:
            ...
            if _tail_role != "assistant":
                ...
                # FORK: upstream's _fill_assistant_tail_content() does not set
                # this. The mutation-footer branch below tests
                # `_footer_tail is _filled_tool_tail_for_persistence` to decide
                # whether it may append the footer to a filled tool-call row;
                # without this assignment that branch is dead and the footer
                # is silently dropped instead of persisted.
        ...
        try:
            if agent._turn_completion_explainer_enabled():
                ...
                if (
                    ...
                    if str(_turn_exit_reason) == "session_persistence_failed":
                        _explanation = agent._format_turn_completion_explanation(
                            _turn_exit_reason,
                            getattr(agent, "_last_persistence_error_cause", None),
                        )
                    else:
                        # The cause refines persistence failures only. Keep
                        # ordinary exits compatible with existing one-argument
                        # formatter overrides and test doubles.
                        _explanation = agent._format_turn_completion_explanation(
                            _turn_exit_reason
                        )
                    if _explanation:
    ...
    if _mutation_footer:
        ...
        if not isinstance(_footer_tail, dict) or _footer_tail.get("role") != "assistant":
            append_message(messages, {"role": "assistant", "content": _mutation_footer})
        elif (
            ...
            if isinstance(_footer_content, str) or _footer_content is None:
                ...
                if not _footer_base.endswith(_mutation_footer):
                    ...
                    # The first turn-end persist above snapshots these exact
                    # dict identities. Once the footer mutates the persisted
                    # tail and clears its marker, the bounded identity scan
                    # must be invalidated too, or the second persist skips the
                    # whole prefix and /resume reloads the footerless row.
                    agent._db_flush_scan_prefix = None
            elif interrupted:
