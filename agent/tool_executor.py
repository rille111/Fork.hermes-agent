...
def _append_unstarted_after_persistence_failure(messages: list, tool_calls) -> None:
    """Pair undispatched tool calls after incremental persistence fails.

    The turn finalizer makes the recovery flush attempt.  Until then, keep the
    in-memory transcript provider-valid without dispatching any more tools or
    claiming that an undispatched call could have produced side effects.
    """
        ...
        messages.append(
            make_tool_result_message(
                ...
                (
                    ...
                    "because session persistence failed]"
                ),
...
class _ToolMutationFenceResult(str):
    """Marker for a mutation skipped behind an abandoned mutation worker."""


_PENDING_FILE_MUTATION_WORKERS: set = set()
_PENDING_FILE_MUTATION_WORKERS_LOCK = threading.Lock()


def _clear_pending_file_mutation_worker(future) -> None:
    with _PENDING_FILE_MUTATION_WORKERS_LOCK:
        _PENDING_FILE_MUTATION_WORKERS.discard(future)


def _remember_pending_file_mutation_worker(future) -> None:
    """Fence every later mutation while an abandoned worker is alive."""
    if future.done():
        return
    registered = False
    with _PENDING_FILE_MUTATION_WORKERS_LOCK:
        if not future.done():
            _PENDING_FILE_MUTATION_WORKERS.add(future)
            registered = True
    if registered:
        future.add_done_callback(_clear_pending_file_mutation_worker)


def _pending_file_mutation_worker_is_running() -> bool:
    """Return whether a previously abandoned mutation can still land late."""
    with _PENDING_FILE_MUTATION_WORKERS_LOCK:
        completed = {
            future for future in _PENDING_FILE_MUTATION_WORKERS if future.done()
        }
        _PENDING_FILE_MUTATION_WORKERS.difference_update(completed)
        return bool(_PENDING_FILE_MUTATION_WORKERS)
        ...
        if block_message is None:
            ...
            def _resolve_pre_tool_block():
                ...
                try:
                    ...
                    # Match model_tools.handle_function_call(): lifecycle hook
                    # framework failures are observational and fail open. A
                    # resolved ``approve`` directive still fails closed inside
                    # resolve_pre_tool_block when its approval gate errors.
                    return None
...
def _run_sequential_tool_execution_middleware(
    ...
    *,
    ...
    if (
        function_name in FILE_MUTATING_TOOL_NAMES
        and _pending_file_mutation_worker_is_running()
    ):
        message = (
            f"[Tool execution skipped — {function_name} was not started because "
            "a previously timed-out file mutation may still be running]"
        )
        logger.warning(
            "file mutation %s blocked behind an abandoned mutation worker",
            function_name,
        )
        trace = middleware_trace if middleware_trace is not None else []
        _emit_terminal_post_tool_call(
            ...
            tool_call_id=tool_call_id,
            status="cancelled",
            error_type="pending_file_mutation",
            error_message=(
                "Tool execution skipped because a prior timed-out file mutation "
                "may still be running"
            ),
            middleware_trace=list(trace),
        )
        return _ManagedToolResult(
            result=_ToolMutationFenceResult(message),
            args=function_args,
            middleware_trace=trace,
            blocked=True,
            dispatched=False,
    ...
    try:
        ...
        if interrupted:
            ...
            future.cancel()
            if function_name in FILE_MUTATING_TOOL_NAMES:
                _remember_pending_file_mutation_worker(future)
            message = (
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    # Defensive barrier for direct/internal callers that bypass the batch
    # planner. Middleware can rewrite distinct model paths to one target, so a
    # structured mutation must never enter this thread pool.
    if any(tc.function.name in FILE_MUTATING_TOOL_NAMES for tc in tool_calls):
    ...
    # The model-level preflight above cannot see through the Tool Search
    # bridge. Re-check the resolved names before starting workers so a direct
    # caller cannot smuggle a structured mutation into the thread pool.
    if any(name in FILE_MUTATING_TOOL_NAMES for _, name, _, _, _, _ in parsed_calls):
        return execute_tool_calls_sequential(
            agent,
            assistant_message,
            messages,
    ...
    # Capture failed file targets in the worker as soon as the tool returns.
    # Post-batch bookkeeping can be delayed by slower sibling tools, during
    # which an external recovery may already change the file.
    mutation_failure_snapshots = [None] * num_tools
    mutation_pre_snapshots = [None] * num_tools
    mutation_execution_arg_holders = [{} for _ in range(num_tools)]
    mutation_completion_order = [None] * num_tools
    mutation_completion_lock = threading.Lock()
    next_mutation_completion = [0]
    turn_generation = getattr(agent, "_file_mutation_turn_generation", None)

    def _mark_mutation_completion(index: int) -> None:
        """Capture real worker completion order without reordering messages."""
        with mutation_completion_lock:
            if mutation_completion_order[index] is None:
                mutation_completion_order[index] = next_mutation_completion[0]
                next_mutation_completion[0] += 1

    def _merge_mutation_baseline(index: int, actual_args: dict, post: Any) -> dict:
        """Prefer a pre-batch baseline when middleware kept target args stable."""
        merged = dict(post) if isinstance(post, dict) else {}
        baseline = mutation_pre_snapshots[index]
        if actual_args == parsed_calls[index][2] and isinstance(baseline, dict):
            for path, snapshot in baseline.items():
                if snapshot is not None:
                    merged[path] = snapshot
        return merged

    for i, (tc, name, args, middleware_trace, block_result, scope_block) in enumerate(parsed_calls):
        if block_result is not None:
            results[i] = (name, args, block_result, 0.0, True, True, middleware_trace)
            continue
        try:
            mutation_pre_snapshots[i] = agent._snapshot_file_mutation_targets(
                name,
                args,
            ...
            logging.debug(
                "file-mutation verifier pre-snapshot failed: %s",
                _ver_err,
            )
            ...
            try:
                def _execute(next_args: dict[str, Any]) -> Any:
                    return agent._invoke_tool(
                        ...
                        observed_args_out=mutation_execution_arg_holders[index],
                    )
                ...
                _mark_mutation_completion(index)
                if getattr(agent, "_file_mutation_turn_generation", None) is turn_generation:
                    try:
                        mutation_args = mutation_execution_arg_holders[index].get(
                            "args",
                            function_args,
                        )
                        mutation_failure_snapshots[index] = _merge_mutation_baseline(
                            index,
                            mutation_args,
                        ...
                        logging.debug(
                            "file-mutation verifier snapshot failed: %s",
                            _ver_err,
                        )
    ...
    try:
        ...
        cancelled_before_start_indices: set[int] = set()
        deadline = time.monotonic() + timeout_s if timeout_s is not None else None
        if runnable_calls:
                        ...
                        for future in not_done:
                            if future.cancel():
                                ...
                                if index is not None:
                                    cancelled_before_start_indices.add(index)
                                    timed_out_indices.discard(index)
                        _still_running = [
                            parsed_calls[i][1]
    ...
    # Freeze one coherent post-time view. A timed-out daemon worker may finish
    # after this point; treating that slot as unknown is safer than letting
    # display timing decide whether its late result updates verifier state.
    post_results = list(results)
    post_observed_dispatches = [
        dict(observed) for observed in mutation_execution_arg_holders
    ]

    # Replay real file outcomes in worker completion order before any display,
    # persistence, or callback code can fail. Middleware may rewrite nominally
    # independent calls onto the same target after the parallelism planner has
    # run, so submission order is not a safe mutation-ledger order.
    mutation_records = []
    for i, (_tc, name, args, _trace, _block_result, _blocked_by_guardrail) in enumerate(parsed_calls):
        r = post_results[i]
        if r is None:
            if i in cancelled_before_start_indices:
                continue
            mutation_args = post_observed_dispatches[i].get("args", args)
            ...
            mutation_records.append((
                mutation_completion_order[i],
                i,
                name,
                mutation_args,
                function_result,
                True,
                mutation_failure_snapshots[i],
            ))
            continue
        (
            function_name,
        ...
        if not blocked:
            observed_dispatch = post_observed_dispatches[i]
            mutation_args = observed_dispatch.get(
                "args",
                function_args,
            )
            if "result" in observed_dispatch:
                mutation_result = observed_dispatch["result"]
                mutation_is_error, _ = _detect_tool_failure(
                    function_name,
                    mutation_result,
                )
            else:
                mutation_result = json.dumps({
                    "error": "File mutation dispatch was not observed; outcome is unknown."
                })
                mutation_is_error = True
            mutation_records.append((
                mutation_completion_order[i],
                i,
                function_name,
    ...
    mutation_records.sort(
        key=lambda record: (
            record[0] is None,
            record[0] if record[0] is not None else record[1],
        ),
    )
    for (
        _completion_order,
        ...
        try:
            ...
            observed_dispatch = post_observed_dispatches[_submission_order]
            if "args" not in observed_dispatch:
                dispatch = DispatchTriState.NOT_DISPATCHED
                raw_result = None
                mutation_is_error = True
            elif "result" not in observed_dispatch:
                dispatch = DispatchTriState.DISPATCHED_NO_RESULT
                raw_result = None
                mutation_is_error = True
            else:
                dispatch = DispatchTriState.DISPATCHED
                raw_result = observed_dispatch["result"]
                mutation_is_error, _ = _detect_tool_failure(
                    function_name,
                    raw_result,
                )
            ...
            if verifier is not None:
                agent._record_file_mutation_result(
                    ...
                    blocked=dispatch is DispatchTriState.NOT_DISPATCHED,
                    effective_task_id=effective_task_id or "default",
                    turn_generation=verifier.generation,
                )
            elif dispatch is not DispatchTriState.NOT_DISPATCHED:
                agent._record_file_mutation_result(
                    function_name,
                    function_args,
                    function_result,
    ...
    for i, (tc, name, args, middleware_trace, _parse_error, _scope_block) in enumerate(
        ...
        effect_disposition = None
        if i in cancelled_before_start_indices and r is None:
            blocked = True
            effect_disposition = "none"
            reason = "the batch timed out" if not agent._interrupt_requested else "the batch was interrupted"
            function_result = (
                f"[Tool execution skipped — {name} was not started before {reason}]"
            )
            _emit_terminal_post_tool_call(
                agent,
                function_name=name,
                ...
                tool_call_id=getattr(tc, "id", "") or "",
                status="cancelled",
                error_type="tool_not_started",
                error_message=function_result,
                middleware_trace=list(middleware_trace),
            )
            tool_duration = 0.0
        elif i in timed_out_indices and r is None:
        ...
        if persistence_failed:
            # Every call in this batch was already dispatched. Preserve its
            # real collected result and effect classification, but do not
            # project unpersisted completion events or retry the failed flush
            # here; the turn finalizer owns the recovery attempt.
            continue
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        if getattr(agent, "_incremental_persistence_failed", False):
            _append_unstarted_after_persistence_failure(
                messages,
        ...
        if agent._interrupt_requested:
            ...
            for skipped_index, skipped_tc in enumerate(remaining_calls):
                ...
                if not _flush_session_db_after_tool_progress(
                    ...
                    _append_unstarted_after_persistence_failure(
                        messages,
                        remaining_calls[skipped_index + 1 :],
                    )
                    return
            break
        ...
        observed_dispatch: Dict[str, Any] = {}
        mutation_failure_snapshots: Dict[str, Any] = {}

        def _capture_mutation_baseline(dispatch_args: Dict[str, Any]) -> None:
            nonlocal mutation_failure_snapshots
            ...
            if verifier is not None:
                if agent._file_mutation_verifier_enabled():
                    verifier.prepare_mutation_dispatch(
                        tool_name=function_name,
                        effective_args=dispatch_args,
                        effective_task_id=effective_task_id or "default",
                        turn_generation=verifier.generation,
                    )
                return
            mutation_failure_snapshots = agent._snapshot_file_mutation_targets(
                function_name,
                dispatch_args,
                effective_task_id or "default",
            )
                ...
                def _execute(next_args: dict) -> Any:
                    ...
                    with suppress_post_tool_call_hook():
                        return _ra().handle_function_call(
                            ...
                            before_dispatch=_capture_mutation_baseline,
                        )
        ...
        _execution_terminal_already_emitted = isinstance(
            function_result,
            (_ToolTimeoutResult, _ToolCancelledResult, _ToolMutationFenceResult),
        )
        _execution_effect_disposition = (
            "unknown"
            if isinstance(function_result, (_ToolTimeoutResult, _ToolCancelledResult))
            else "none"
            if isinstance(function_result, _ToolMutationFenceResult)
            else None
        )
        ...
        _executor_must_emit_post_hook = (
            ...
            and not _execution_terminal_already_emitted
        )
        ...
        try:
            ...
            mutation_args = observed_dispatch.get("args", function_args)
            if "args" not in observed_dispatch:
                dispatch = DispatchTriState.NOT_DISPATCHED
                raw_result = None
                mutation_is_error = True
            ...
            if verifier is not None:
            ...
            elif not _execution_blocked and dispatch is not DispatchTriState.NOT_DISPATCHED:
                legacy_result = (
                    raw_result
                ...
                agent._record_file_mutation_result(
                    ...
                    mutation_is_error,
                    task_id=effective_task_id or "default",
                    failure_snapshots=mutation_failure_snapshots,
                )
        except Exception as _ver_err:
            logging.debug("file-mutation verifier record failed: %s", _ver_err)
...
def execute_tool_calls_segmented(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, segments=None) -> None:
    ...
    *k+1..n* without executing them while preserving one result per
    ...
    segments = [(kind, list(calls)) for kind, calls in segments]
    for segment_index, (kind, calls) in enumerate(segments):
        if getattr(agent, "_incremental_persistence_failed", False):
            _append_unstarted_after_persistence_failure(
                ...
                (
                    call
                    for _, remaining_calls in segments[segment_index:]
